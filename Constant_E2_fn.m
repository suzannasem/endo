%%%% Incorporating E2 as constant values %%%%

%%% function file %%%

function out = Constant_E2_fn(t,Y, beta1, omega, E2, P4, K1, K2)
%% parameters %%
...
dm1dt = beta1 .* (ef + ea).* m0 + thetaM.*(ka ./ (cka + ka)).* m0 + beta21.* (1 + (phi4 .* (K1 .* Dab).^n4 ./ ((K1 .* Dab).^n4 + H4.^n4))).* m2 - beta12.*(1 + (phi3 .* Da.^n3 ./ (Da.^n3 + H3.^n3))) .* m1 - deltaM1 .* m1;
dm2dt = beta2 .* m0 + beta12 .* (1 + (phi3 .* (K2 .* Da).^n3 ./ ((K2 .* Da).^n3 + H3.^n3))).* m1 - beta21 .*(1 + (phi4 .* Dab.^n4 ./ (Dab.^n4 + H4.^n4))) .* m2 - deltaM2 .* m2;

% nk cells
dk0dt = (1 + (phi1 .* E2.^n1)).*muk - thetaK .* k0 .* (m1 ./ (cm1 + m1)) - deltaK0 .* k0;
dkadt = thetaK .* k0 .* (m1 ./ (cm1 + m1)) + etaK .* (1 - (ka + k0)./kc).* ka - sigma .* (ef + ea).* ka - deltaKa * ka;

% endometrial cells
de0dt = mue - deltaE0 .* (rho0 .* e0 + (1-rho0).* e0);
defdt = deltaE0 .* rho0 .* e0 - (phi2 .* H2.^n2 ./ ((K1 .*Rb).^n2 + H2.^n2)) .* omega .*(gamma .* ka + (1-gamma).* m1).* ef - rhoF .* ef - deltaEf .* ef;
deadt = rhoF .* ef + etaE .* ea .* (1 - (ea ./ ec)) .* (m2 ./ (cm2 + m2)) - (phi5 .* H5.^n5 ./ ((K1 .* Rb).^n5 + H5.^n5)) .* omega.* (gamma.* ka + (1-gamma).* m1).*ea - deltaEa .* ea;

% receptor dynamics (unscaled)
